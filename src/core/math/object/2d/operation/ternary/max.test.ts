import type { Object2D } from '../../../../../../types'
import { zeroObject } from '../../../../../util/object'
import { maxObject2d, maxObject2dX, maxObject2dY } from './max'

const VECTOR_A: Object2D = { x: 5, y: 2 }
const VECTOR_B: Object2D = { x: 1, y: 8 }

test('max vector', () => {
  const a: Object2D = { ...VECTOR_A }
  const b: Object2D = { ...VECTOR_B }

  // Result of vector maximum
  expect(maxObject2d(a, b, zeroObject())).toStrictEqual({ x: 5, y: 8 })

  // No mutations
  expect(a).toStrictEqual(VECTOR_A)
  expect(b).toStrictEqual(VECTOR_B)
})

test('max vector x', () => {
  const a: Object2D = { ...VECTOR_A }
  const b: Object2D = { ...VECTOR_B }

  // Result of vector maximum
  expect(maxObject2dX(a, b, zeroObject())).toStrictEqual({ x: 5, y: 0 })

  // No mutations
  expect(a).toStrictEqual(VECTOR_A)
  expect(b).toStrictEqual(VECTOR_B)
})

test('max vector y', () => {
  const a: Object2D = { ...VECTOR_A }
  const b: Object2D = { ...VECTOR_B }

  // Result of vector maximum
  expect(maxObject2dY(a, b, zeroObject())).toStrictEqual({ x: 0, y: 8 })

  // No mutations
  expect(a).toStrictEqual(VECTOR_A)
  expect(b).toStrictEqual(VECTOR_B)
})
